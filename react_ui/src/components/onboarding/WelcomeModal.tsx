...
export const WelcomeModal: React.FC<{
  ...
  const handleStartTour = (tourId: string) => {
    const tour =
      tourId === "app-workflow" ? APP_WORKFLOW_TOUR : TEMPLATE_EDITOR_TOUR;
    updateSettings({ hasSeenWelcome: true });

    // Synchronize routing based on the selected tour
    if (onNavigate) {
      if (tourId === "template-editor") {
        onNavigate("template_editor");
      } else if (tourId === "app-workflow") {
        onNavigate("import");
      }

      // Allow time for navigation and render before starting the tour overlay
      setTimeout(() => {
        startTour(tour);
      }, 150);
    } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[10003] flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 border border-gray-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-300 text-sm mt-1">
                LogiQore Reporter for Geological Data
              </p>
            </div>
          </div>

          <p className="text-gray-300 leading-relaxed">
            Let's get you started with a quick guided tour! Choose which part of
            the application you'd like to explore first.
          </p>
        </div>
